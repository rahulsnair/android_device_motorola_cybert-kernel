 ...
 *
 * Private Header for Performance Monitor.
 */

#ifndef _MOT_PERF_MON_PRIVATE_H_
#define _MOT_PERF_MON_PRIVATE_H_
...
struct cpu_perf_info {
	...
	enum mot_perf_cpu_idle_state idle_state;
	...
	struct mot_event_data perf_ev_data[PERF_NUM_COMMON_EVS];
};

/**
 * struct mot_perf_mon_state - Container for Monitor Metadata.
 *
 * This struct contains internal mot_perf_mon meta data
 * It should be not be exposed to files outside of mot_perf_mon.
 *
...
struct mot_perf_mon_state {
	...
	struct mot_cpu_perf_data *client_shared_data;
	struct task_struct *perf_mon_task;
	struct cpu_perf_info *cpu_data_arr;
};

/**
 * struct mot_perf_mon_configs - Container for Monitor Configuration Data.
 *
