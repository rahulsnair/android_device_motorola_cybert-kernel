...
TRACE_EVENT(sched_select_task_rq,
	...
	TP_fast_assign(
		...
		__entry->select_reason     = select_reason;
	...
	TP_printk(
		"pid=%4d 32-bit=%d in_irq=%d select_reason=0x%08x backup_reason=0x%04x pre-cpu=%d target=%d util=%d util_est=%d uclamp=%d mask=0x%lx eff_softmask=0x%lx latency_sensitive=%d sync=%d cpuctl=%d cpuset=%d",
		__entry->pid,
		__entry->compat_thread,
		__entry->in_irq,
		__entry->select_reason,
