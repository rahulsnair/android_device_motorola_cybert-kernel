...
/*
 * Copyright (C) 2025 Motorola Mobility Inc.
 *
 * Common Header for Performance Monitor
 */

#ifndef _MOT_PERF_MON_H_
#define _MOT_PERF_MON_H_

/**
 * mot_perf_event_idx - A type for performance events.
 *
 * We expect a particular event to have
 * the same index across all ids and data arrays.
*/
enum mot_perf_event_idx {
...
/**
 * mot_perf_cpu_idle_state - Enum for idle states.
 *
 * Used to supply clients cpu idle information.
*/
enum mot_perf_cpu_idle_state {
	PERF_CPU_ACTIVE,
	PERF_CPU_IDLE_C1,
	PERF_CPU_IDLE_C2
};
 ...
 */
struct mot_event_data {
	struct perf_event *pevent;
	enum mot_perf_event_idx element_idx;
	enum mot_counter_type counter_type;
...
/**
 * struct mot_cpu_perf_data - Container for per-cpu counter data.
 *
 ...
 */
struct mot_cpu_perf_data {
	bool cpu_mon_on;
	ktime_t last_update_ts;
	unsigned long cpu_freq;
	unsigned long time_delta_us;
	unsigned long perf_ev_last_delta[PERF_NUM_COMMON_EVS];
	enum mot_perf_cpu_idle_state cpu_idle_state;
};

/**
 * mot_perf_mon_callback_func_t - A type for client callbacks.
 *
 * @mot_cpu_perf_data_arr:	We supply a copy of all CPU cores
 * @private_data:		Private data for the client metadata.
 */
typedef void (*mot_perf_mon_callback_func_t)(struct mot_cpu_perf_data* mot_cpu_perf_data_arr,
						void *private_data);

/**
 * struct mot_perf_mon_client - Callback data for a client.
 *
 ...
 */
struct mot_perf_mon_client {
	struct list_head node;
	const char *name;
	void* private_data;
	mot_perf_mon_callback_func_t client_callback;
};

#if IS_ENABLED(CONFIG_MEMLAT_MON)

/***
 * mot_perf_mon_get_data - Primary function for retrieving cpu performance data.
 *
 * Inputs:
...
*/
int mot_perf_mon_get_data(unsigned int cpu, struct mot_cpu_perf_data *data_dest);
...
/**
 * mot_perf_mon_add_client - Registers a client for the perf monitor to service.
 *
 * Input:
 ...
 *
 * Side-Effects:	May call mot_perf_mon_start on first client.
*/
int mot_perf_mon_add_client(struct mot_perf_mon_client *client);

/**
 * mot_perf_mon_remove_client - Unregisters a client for the perf monitor to service.
 *
 * Input:
...
*/
void mot_perf_mon_remove_client(struct mot_perf_mon_client *client);

/**
 * mot_perf_mon_tick_update_counters - Updates performance counters and triggers
 *				      latency governor servicing.
 *
 * Requirements:		IRQs must be off on local cpu.
*/
void mot_perf_mon_tick_update_counters(void);

/**
 * mot_perf_mon_update_clients - Checks and updates monitor clients.
 *
 * Side-Effects:	Could call client servicing function.
*/
void mot_perf_mon_update_clients(void);

#else

/* No-op functions if monitor is unused.*/
static inline int mot_perf_mon_get_data(unsigned int cpu, struct mot_cpu_perf_data *data_dest)
{
	return 0;
}
...
static inline int mot_perf_mon_add_client(struct mot_perf_mon_client *client)
{
	return 0;
}
static inline void mot_perf_mon_remove_client(struct mot_perf_mon_client *client)
{
	return;
}
static inline void mot_perf_mon_tick_update_counters(void)
{
	return;
}
static inline void mot_perf_mon_update_clients(void)
{
	return;
}
