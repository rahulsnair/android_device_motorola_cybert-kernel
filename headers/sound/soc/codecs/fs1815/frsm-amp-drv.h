...
static struct frsm_amp *frsm_amp_get_pdev(void);
static int frsm_amp_notify(struct frsm_amp *frsm_amp, int event,
		struct frsm_argv *argv);
static int frsm_amp_set_tuning(struct frsm_amp *frsm_amp,
		struct frsm_argv *argv);
static int frsm_amp_set_calre(struct frsm_amp *frsm_amp,
		struct frsm_argv *argv);
static int frsm_amp_get_livedata(struct frsm_amp *frsm_amp,
		struct frsm_argv *argv);
static int frsm_amp_set_fsalgo(struct frsm_amp *frsm_amp,
		struct frsm_adsp_pkg *pkg);
static int frsm_amp_mntr_switch(struct frsm_amp *frsm_amp, bool enable);
static int frsm_amp_prot_battery(struct frsm_amp *frsm_amp, bool restore);

static void frsm_amp_register_notify_callback(int (*func)(
		int event, void *buf, int size));
static int frsm_amp_send_event(int event, void *buf, int size);
static int frsm_amp_init_dev(int spkid, bool force);
static int frsm_amp_set_scene(int spkid, int scene);
static int frsm_amp_spk_switch(int spkid, bool on);
